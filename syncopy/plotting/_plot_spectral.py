...
def singlepanelplot(self, trials="all", channels="all", tapers="all",
    ...
    if not isTimeFrequency:
        ...
        time = self.time[trList[0]][self.selection.time[0]]
...
def _compute_pltArr(self, nFreq, N, nTime, complexConversion, pltDtype,
    ...
    if trial == "all":
        trList = self.selection.trials
    else:
        trList = [trial]
    nTrls = len(trList)
    useFancy = self.selection._useFancy
    if channel == "all":
        idx[chanIdx] = self.selection.channel
    else:
        idx[chanIdx] = np.where(self.channel == channel)[0]
        useFancy = True
    if freq ==  "all":
        idx[freqIdx] = self.selection.freq
    else:
        idx[freqIdx] = np.where(self.freq == freq)[0]
        useFancy = True
    if taper == "all":
        idx[taperIdx] = self.selection.taper
    else:
        idx[taperIdx] = [taper]
        useFancy = True
    ...
    for tk, trlno in enumerate(trList):
        ...
        idx[timeIdx] = self.selection.time[tk]
        if not useFancy:
...
def _prep_spectral_plots(self, name, **inputArgs):
    ...
    self.selection = {"trials": inputArgs["trials"],
    ...
    trList = self.selection.trials
    nTrials = len(trList)
    chArr = self.channel[self.selection.channel]
    nChan = chArr.size
    freqArr = self.freq[self.selection.freq]
    nFreq = freqArr.size
    tpArr = np.arange(self.taper.size)[self.selection.taper]
    nTap = tpArr.size
